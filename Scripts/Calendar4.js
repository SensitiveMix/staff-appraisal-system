...
L_calendar.prototype = {
    ...
    CreateHTML: function () {
        ...
        stylestr += "#L_calendar-today{cursor:pointer;float:left;width:63px;height:20px;line-height:20px;margin:1px;text-align:center;background:lightblue}"
        stylestr += "</style>";
    ...
    Today: function ()  //Today Button
    {
        ...
        { this.L_TheMonth = "0" + this.L_TheMonth; }
        today = new Date().getDate();
        if (this.InputObject) {
            this.InputObject.value = this.L_TheYear + "-" + this.L_TheMonth + "-" + today + " 15:00";
        }
        this.CloseLayer();
        try {
            setTitle();
    ...
    DayClick: function (mm, dd)  //点击显示框选取日期，主输入函数*************
    {
        ...
        if (this.ClickObject) {
            if (!dd) { return; }
            if (dd < 10) { dd = "0" + dd; }
            this.InputObject.value = yy + "-" + mm + "-" + dd + " 15:00"; //注：在这里你可以输出改成你想要的格式
            this.CloseLayer();
            try {
                setTitle();
            } catch (e) {
